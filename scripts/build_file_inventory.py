...
Output: data/recon/file_inventory.duckdb

Usage:
  python3 scripts/build_file_inventory.py --all           # full rebuild (drop + recreate)
  python3 scripts/build_file_inventory.py --source ASOS   # replace only ASOS rows
  python3 scripts/build_file_inventory.py --source GHCN_daily
"""
...
# Source definitions
# ---------------------------------------------------------------------------
# Each entry maps a source label → directory path(s) relative to BASE.
# 'skip_subdirs': subdirectory names to prune during os.walk for this source.
SOURCES = {
    'ASOS': {
        'paths': [os.path.join('data', 'ASOS')],
        'skip_subdirs': {'raw_temp', 'asos_raw_downloads', 'raw_downloads', 'temp_extracted'},
    },
    'GHCNd': {
        'paths': [os.path.join('data', 'GHCNd')],
    },
    'GHCNh': {
        'paths': [os.path.join('data', 'GHCNh')],
    },
    'HOMR': {
        'paths': [os.path.join('data', 'HOMR')],
    },
    # Removed: HPD_1hr, NCEI_15min_raw, GHCN_daily (old paths no longer exist)
}
...
STATION_ID_PATTERNS = [
    ...
    # Current NCEI access layout:
    # e.g. asos-1min-pg2-KABE-202401.parquet → ICAO = KABE
    (re.compile(r'^asos-1min-pg2-([A-Z0-9]{4})-(\d{6})\.parquet$', re.IGNORECASE), 'ICAO_ASOS'),
    ...
    # ── GHCN parquet (from ghcnd_all_parquet): same ID, .parquet ─────────
    # Mixed-case + underscores: CoCoRaHS stations like US10perk005, US10box_005
    (re.compile(
        r'^((?:US|RQ|VQ|CQ|GQ|AQ|MQ|FQ|SQ|RW)[A-Za-z0-9_]{9})\.parquet$',
        re.IGNORECASE
    ), 'GHCND_DLY'),
    ...
    for pattern, fmt in STATION_ID_PATTERNS:
        ...
        if m:
            ...
            if fmt == 'ICAO_ASOS':
                ...
                return code if len(code) == 5 else f"0{code}", fmt, False
            if fmt == 'USAF_WBAN':
...
def walk_source(source_name: str, cfg: dict, intl_skipped_ref: list):
    ...
    intl_skipped_ref is a single-element list used as a mutable counter.
    """
